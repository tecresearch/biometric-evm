...
public:
    ...
    void handleEnrollment() {
        ...
        if (fingerprint.isEmpty()) {
            displayAndLog("Capture failed!", "Try again");
            return;
        }
    ...
    void handleVerification() {
        ...
        if (currentVoterId.isEmpty()) {
            displayAndLog("Verification Failed!");
            delay(2000);
            setState(SystemState::MENU);
            return;
        }
    ...
    // --- Vote API Call ---
    String voteEndpoint = String(serverUrl) + "/api/candidates/" + candidateId + "/vote";
    HTTPClient http;
    ...
    if (voteCode == HTTP_CODE_OK || voteCode == HTTP_CODE_CREATED) {
        ...
        // --- Mark Voter API Call ---
        String markEndpoint = String(serverUrl) + "/api/voters/" + voterId + "/mark-voted";
        ...
        displayAndLog("VOTE FAILED", "Try again later");
    }
